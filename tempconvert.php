...
//  )) {
    ...
    if(empty($_POST['unit_1']) && empty($_POST['unit_2']) ) { // if both are empty
        $errorMsg .= '<span class="error">Please select your both your units of measurement!</span>';
    }

    

    if(empty($_POST['unit_1']) && isset($_POST['unit_2'])) { // if unit 1 is null but unit 2 is good
        $errorMsg .= '<p>Please select your unit of measurement!<p>';
    }
    

    if(empty($_POST['unit_2']) && isset($_POST['unit_1'])) { // if unit 2 is null but unit 1 is good
        $errorMsg .= '<span class="error">What are we converting to?</span>';
    }


    if($_POST['temp'] === "") { // if temp is empty
        $errorMsg .= '<span class="error">Please fill out your temp!</span>';
    }
   ...
   if(isset(
     ...
     $temp = intval($_POST['temp']); 
        ...
        $errorMsg .= '<p>Please Select Two Different Units!</p>';
        }
        ...
        if($unit_1 == 'Kelvin' && $unit_2 == 'Fahrenheit') { // Kelvin to Fahrenheit	° F = 9/5 (K - 273) + 32
            $converted_temp = 9/5 * ($temp - 273) + 32;
        }

        if($unit_1 == 'Fahrenheit' && $unit_2 == 'Kelvin') { // Fahrenheit to Kelvin	K = 5/9 (° F - 32) + 273
            $converted_temp = 5/9 * ($temp - 32) + 273;
        }

        if($unit_1 == 'Kelvin' && $unit_2 == 'Celsius') { // Kelvin to Celsius	 ° C = K - 273
            $converted_temp = $temp - 273;
        }

        if($unit_1 == 'Celsius' && $unit_2 == 'Kelvin') { // Celsius to Kelvin	 K = ° C + 273
            $converted_temp = $temp + 273;
        } 
            // echo '
            // <div class="result">
            ...
            // if(isset($_POST['unit_1']) === $_POST['unit_2']) {
            //     echo '<p>Please Select Two Different Units!</p>';
            // }
    } // of isset if statment
